...
Deployed on free tiers (Vercel + Render) as a temporary demo, expected to stay up for about a week from 2026-09-03. Its auto-refresh interval is set to 15 seconds (vs. the app's 5-minute default) specifically so it's easy to watch react on screen without a long wait. Two things to expect:
...
- **Auto-refresh lives on the server, not as frontend polling.** `RateRefreshBackgroundService` (an `IHostedService`) ticks on a timer and calls the exact same `IRateService.RefreshAsync` that the "Refresh Rates" button calls — so a scheduled tick drives the identical `RatesRefreshedEvent` → alert evaluation → SignalR push pipeline as a manual refresh. This keeps the "self-refreshing app" requirement satisfied without contradicting "event-driven, not polling": one job refreshes for every connected client at once, instead of N browser tabs each independently asking "anything new?" on their own timers. Interval defaults to 5 minutes (`RateRefresh:IntervalMinutes`), or set `RateRefresh:IntervalSeconds` for finer-grained control (used for the live demo below — see why 15s, not lower). We don't recommend going much below that: Frankfurter's underlying ECB reference rates only update once a day, so refreshing every few seconds buys no real freshness, only extra load on a free public API we don't pay for.
...
- **Server validation errors surface at the field, not just as generic text.** `ApiError` carries the backend's structured `errors` dictionary (ASP.NET Core's `ValidationProblemDetails`, keyed by property name) alongside a human-readable message built from the actual field message(s), not the generic "One or more validation errors occurred." title. `AddItemForm` maps `BaseCurrency`/`QuoteCurrency` keys straight to the matching input, giving it the same red-border treatment as client-side format errors — so "USD/ZZZ is rejected" visibly points at the quote field, not the base one, without the form needing to know *why* the currency was invalid, just *which* field the server complained about.
- **A shared toast (`ToastProvider`/`useToast`), not a per-form ad-hoc error string.** Every create-form catch block calls the same `showToast(message, "error")` instead of rendering its own inline error text — one visible, dismissible, auto-expiring notification pattern app-wide rather than three slightly different ones. No new dependency: it is a ~50-line context provider rendering a fixed-position stack, verified in `ToastProvider.test.tsx` and end-to-end in Playwright.
