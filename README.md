...
- **Two kinds of backend tests, deliberately.** `Application.Tests` unit-tests business logic in isolation (services, validators, event handlers) with every dependency mocked via NSubstitute — fast, and pinpoints exactly what broke. `Api.Tests` are real integration tests through `WebApplicationFactory` against an isolated in-memory SQLite database, with only the one genuinely external thing (`IRateProvider`) swapped for a fake — so the real DI graph, EF Core mappings, and HTTP pipeline are all actually exercised, not assumed.
...
```bash
dotnet test                                                  # 77 tests: unit (Application) + integration (Api, isolated SQLite + mocked rate provider + resilience pipeline)
