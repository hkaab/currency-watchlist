...
describe("CreateAlertForm", () => {
  it("prompts to add a pair first when there are no items", () => {
    renderWithToast(<CreateAlertForm items={[]} onCreate={vi.fn()} />);
  ...
  it("keeps submit disabled until a positive threshold is entered", async () => {
    renderWithToast(<CreateAlertForm items={items} onCreate={vi.fn()} />);
  ...
  it("selects the newly available item once one is added after mount", async () => {
    ...
    const { rerender } = renderWithToast(<CreateAlertForm items={[]} onCreate={onCreate} />);
  ...
  it("calls onCreate with the selected item, condition, and threshold", async () => {
    const onCreate = vi.fn().mockResolvedValue(undefined);
    renderWithToast(<CreateAlertForm items={items} onCreate={onCreate} />);
  ...
  it("shows a toast when creation fails", async () => {
    const onCreate = vi.fn().mockRejectedValue(new ApiError("Watchlist item not found.", 404));
    renderWithToast(<CreateAlertForm items={items} onCreate={onCreate} />);

    await userEvent.type(screen.getByLabelText("Threshold"), "1.6");
    await userEvent.click(screen.getByRole("button", { name: "Create alert" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("Watchlist item not found.");
  });
});
